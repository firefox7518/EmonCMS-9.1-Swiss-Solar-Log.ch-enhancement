...
function pvvertical_widgetlist()
{
	var widgets =
	{
		"pvvertical":
		{
			"offsetx":-80,"offsety":-80,"width":119,"height":225,
			"menu":"Widgets",
			"options":    [],
			"optionstype":[],
			"optionsname":[],
...
//	addOption(widgets["pvvertical"], "colour",      "colour_picker", _Tr("Colour"),          _Tr("Colour to draw bar in"),                                                       []);
	addOption(widgets["pvvertical"], "graduations", "dropbox",       _Tr("Graduations"),     _Tr("Should the graduations be shown"),                                             graduationDropBoxOptions);
	addOption(widgets["pvvertical"], "gradNumber",  "value",         _Tr("Num Graduations"), _Tr("How many graduation lines to draw (only relevant if graduations are on)"),     []);
...
function pvvertical_init()
{
	setup_widget_canvas('pvvertical');
}

function pvvertical_draw()
{
...
function pvvertical_slowupdate()
{

}

function pvvertical_fastupdate()
{
	pvvertical_draw();
}
				...
				graduationQuant)
{
	...
	var pvvertical_border_space = 10;
	var pvvertical_top = ((height-pvvertical_border_space) - (scaled_value * (height - (pvvertical_border_space*2))));

	if (pvvertical_top < pvvertical_border_space)		// Clamp value so we don't overshoot the top of the bargraph.
		pvvertical_top = pvvertical_border_space;

	context.fillRect(pvvertical_border_space,
					pvvertical_top,
